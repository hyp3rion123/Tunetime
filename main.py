...
def build_playlist():
    ...
    userSongs = [request.args.get("fsong"), request.args.get("lsong")]
    for i in range(2):
        userSongs[i] = search_song(token, userSongs[i])
        # Add song genre too - used for song similarity comparison
        userSongs[i]["song_genre"] = get_artist_genres(token, userSongs[i]["artist_id"])
    # MIN STEPS = 2
    step_count = int(request.args.get("steps"))
    print(get_song_feature(token, userSongs[0]["song_id"]))
    print(get_song_feature(token, userSongs[1]["song_id"]))
    step_features = get_step_features(
        get_song_feature(token, userSongs[0]["song_id"]),
        get_song_feature(token, userSongs[1]["song_id"]),
        step_count,
    )
    print("step_features: ", step_features)
    # Get recommendations based on step features
    current_song = userSongs[0]
    songs = [current_song]
    song_list_names = [userSongs[0]["song_name"], userSongs[1]["song_name"]]
    for i in range(step_count - 2):
        if i > ((step_count - 2) / 2):
            current_song["song_genre"] = [
                current_song["song_genre"],
                userSongs[1]["song_genre"],
            ]
        ...
        current_song = select_unchosen_song(
            token, potential_songs, song_list_names, step_features[i]
        )
        # Similarity scores
        current_song["similarity_score_first"] = round(
            get_similarity_score(
                token, userSongs[0]["song_id"], current_song["song_id"], None
            ),
            3,
        )
        current_song["similarity_score_last"] = round(
            get_similarity_score(
                token, userSongs[1]["song_id"], current_song["song_id"], None
            ),
            3,
        )
        songs.append(current_song)
        song_list_names.append(current_song["song_name"])
    current_song = userSongs[1]
    songs.append(current_song)
    ...
    return {"songs": songs, "steps": step_count, "user_id": user_id, "playlist_id": playlist_create_response["id"]}
